...
/** Simulate / sync product-usage segments (placeholder until BMGenie API hookup) */
router.post(
  '/generate-segments',
  requirePermission('leads:generate_segments'),
  (req, res) => {
    const samples = req.body?.leads;
    if (!Array.isArray(samples) || !samples.length) {
      return res.status(400).json({
        error: 'Provide leads array',
        example: [
          {
            name: 'Alex Photo Co',
    ...
    const insert = db.prepare(
      `INSERT INTO leads (id, name, email, phone, company, country, source, bmgenie_user_id, estimated_value, notes)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    );
    const findDup = db.prepare(
      `SELECT id FROM leads WHERE email = ? COLLATE NOCASE AND source = ?`,
    );

    const tx = db.transaction((rows) => {
      for (const row of rows) {
        if (!row.name || !LEAD_SOURCES[row.source]) {
          skipped += 1;
          continue;
        }
        if (row.email) {
          const dup = findDup.get(row.email, row.source);
          if (dup) {
            skipped += 1;
            continue;
          }
        ...
        insert.run(
          ...
          row.country || null,
          row.source,
          row.bmgenie_user_id || null,
          Number(row.estimated_value) || 0,
          row.notes || `Auto-generated: ${LEAD_SOURCES[row.source].label}`,
        );
        db.prepare(
          `INSERT INTO lead_activities (id, lead_id, user_id, type, summary)
           VALUES (?, ?, ?, 'segment_generated', ?)`,
        ).run(uuid(), id, req.user.id, `Segment lead: ${LEAD_SOURCES[row.source].label}`);
        created += 1;
      }
    });
    tx(samples);
